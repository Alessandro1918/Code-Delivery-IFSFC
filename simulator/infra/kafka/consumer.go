...
// It also has a Message channel which is a channel where the messages are going to be pushed
...
func (k *KafkaConsumer) Consume() {
	...
	//keeps listening while running:
	for {
		msg, err := c.ReadMessage(-1)
		if err == nil {
			k.MsgChan <- msg		//sends message to Message Channel
		}
	}
}